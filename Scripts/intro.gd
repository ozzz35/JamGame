...
func play():
	switch_video("video")
	clear_all()
	await display_text("The Earth, 1979", 1)
	
	await get_tree().create_timer(default_wait_time).timeout
	
	clear_all()
	await display_text("The world has been struck by a major pandemic.", 3)
	
	await get_tree().create_timer(default_wait_time).timeout
	
	clear_all()
	await display_text("The World Government is trying its hardest ", 2)
	
	await display_text("to remedy this disaster.", 4)
	
	await get_tree().create_timer(default_wait_time).timeout
	
	clear_all()
	await display_text("You are an official and", 1)
	await display_text("the last link in the chain of command.", 3)
	...
	clear_all()
	await display_text("Follow orders", 5)
	await screen_flash(5)
	
	await get_tree().create_timer(short_wait_time).timeout
	
	display_text("And save the Earth.", 5)
	
	await get_tree().create_timer(0.5).timeout
	
	await reset_camera(2)
